...
class cube_ins{
    constructor(meshmode,name){
        ...
        this.cube.position.set(Math.sin(Math.random()+this.random*3+2)*11,this.random*13,Math.cos(Math.random()+this.random*3+2)*11);
...
function tick() {
    ...
    if(intersects.length>0){cubes[intersects[0].object.name].tosize=3;looker=intersects[0];}
    else{looker=null;}
